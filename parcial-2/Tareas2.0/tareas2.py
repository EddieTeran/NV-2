...
class ListaTareas:  # Nombre de la clase  ListaTareas
    def __init__(self, master):
        ...
        self.master.bind("<Shift-C>", lambda e: self.mark_completed())
        self.master.bind("<Shift-D>", lambda e: self.delete_task())
        self.master.bind("<Escape>", lambda e: self.master.quit())

        # Configurar estilos para feedback visual
        self.style = ttk.Style()
        self.style.configure("Pending.Treeview", background="white")
        self.style.configure("Completed.Treeview", background="lightgreen")
    ...
    def add_task(self):
        ...
        if task:
            ...
            item = self.task_list.insert("", "end", values=(task, "Pendiente"))
            self.task_list.item(item, tags=("pending",))
    ...
    def mark_completed(self):
        ...
        if selected_item:
            ...
            if new_status == "Completada":
                self.task_list.item(selected_item, tags=("completed",))
            else:
                self.task_list.item(selected_item, tags=("pending",))
